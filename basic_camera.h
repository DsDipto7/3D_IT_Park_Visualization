...
public:
    ...
    glm::mat4 createProjectionMatrix(float aspect, float nearPlane = 0.1f, float farPlane = 250.0f) const {
        return glm::perspective(glm::radians(Zoom), aspect, nearPlane, farPlane);
    }

    // Floor height constraint for walking
    void setFloorHeight(float floorY, float dt) {
        float EyeHeight = 1.75f;
        float targetY = floorY + EyeHeight;
        float blend = glm::clamp(dt * 10.0f, 0.0f, 1.0f);
        eye.y += (targetY - eye.y) * blend;
    }

    // Reset camera
    void reset(const glm::vec3& pos, const glm::vec3& target) {
        eye = pos;
